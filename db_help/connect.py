    ...
    def _conn(self, spec):
        try:
            db = pymysql.connect(host=spec["host"], port=spec["port"], user=spec["username"],
                                 password=spec["password"], database=spec["database"],
...
class DataClient:
    ...
    def get_db(self):
        """
       : 需要连接的数据库, 第一次引入时需要显示get
        """

        db = DataBase(self.conf, debug=self.debug)
        self.conns.setdefault(self.conf.get("database"), db)
        return db
